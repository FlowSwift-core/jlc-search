...
重要：避免部分匹配问题！
- 搜索 10K 时，使用 mfr LIKE '%1002%' 而不是 mfr LIKE '%10K%'
- 搜索 100K 时，使用 mfr LIKE '%1003%' 而不是 mfr LIKE '%100K%'
- 搜索 1K 时，使用 mfr LIKE '%1001%' 而不是 mfr LIKE '%1K%'
- 搜索 1M 时，使用 mfr LIKE '%1004%' 而不是 mfr LIKE '%1M%'

重要：不要依赖 description 字段！
- 很多元件的 description 为空
- 优先使用 mfr 和 package 字段进行匹配
- 公差、电压等参数可以在 extra 字段中搜索

示例：
用户：10K 0603 1%
SQL：SELECT lcsc, mfr, package, stock, basic, preferred, price, datasheet FROM components WHERE mfr LIKE '%1002%' AND package LIKE '%0603%' ORDER BY basic DESC, preferred DESC, stock DESC LIMIT 5

用户：100nF 电容 50V
SQL：SELECT lcsc, mfr, package, stock, basic, preferred, price, datasheet FROM components WHERE mfr LIKE '%104%' AND package LIKE '%0603%' ORDER BY basic DESC, preferred DESC, stock DESC LIMIT 5
